...
- Красная «загрузить»: skip inbound только если проба живая, не слабее keep, диск ≥ max(проба, keep), нет extra. Сорванная/слабая проба не закрывает cid и не затирает `lessonsAlfa`. `keepAlfaProbe` — ядро A.
