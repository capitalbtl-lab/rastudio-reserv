...
Правки из этой среды сами уходят в `main` через ~20 секунд тишины (`scripts/watch-push.mjs`). Сайт на Beget подтягивает `main` раз в минуту (`rastudio-deploy` в PM2). SSH больше не нужен.
