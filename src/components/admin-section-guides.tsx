...
export function AdminSectionGuides() {
  ...
  async function save() {
    ...
    if (res.ok && "guides" in res) {
      ...
      setMsg(
            on
              ? guide.id === "roles" || guide.id === "site"
                ? "Сохранено. Олег и Ольга читают эту инструкцию на сайте."
                : "Сохранено. Голос админки читает эту инструкцию в кабинете."
              : "Сохранено. Инструкция выключена — агент её не видит.",
          );
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-2 max-w-2xl text-sm text-muted">
          Единственный источник правил раздела. Сопоставление — только по ID. Правка «Роли» или «Сайт · запись» сразу попадает в чат Олега и Ольги (если стоит текущий REV).
        </p>
      </div>
          ...
          <article className="rounded-3xl bg-surface p-5 shadow-[var(--shadow-border)] md:p-6" data-guide-id={guide.id} data-guide-rev={GUIDE_REV}>
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <label className="flex items-center gap-2 rounded-full bg-surface-2 px-3 py-1.5 text-sm">
                ...
                <InfoTip text="Включено: Олег/Ольга читают «Роли» и «Сайт · запись»; голос админки читает все включённые разделы. Выключено — этот текст агенту не отдаётся." />
              </label>
            </div>
          </article>
