...
const HINT = {
  ...
  loadOneMoney: "Красная кнопка идёт по ученикам слева по одному. Кассу этого человека дочитывает до конца: если Alfa отдала не все страницы или не ответила — пауза 5 с и снова тот же id, к следующему не переходит. Направо только если остаток на диске совпал с шапкой Alfa ±1 ₽. Если страницы дочитаны, а рубль не сошёлся — карточка остаётся слева. Календарь подгружает, только если его ещё нет. Окно лет режет занятия, не платежи. В Alfa оплаты не создаёт. Стоп прерывает очередь.",
