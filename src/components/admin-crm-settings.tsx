...
const HINT = {
  ...
  archPupils: "Смотрит карточки учеников той выборки, что на шаге 1: «Сейчас ходят» или рабочий архив — в одном прогоне не смешивает. Собирает номера групп, где они числились. Живые группы из этого списка отбрасывает. Остальные — архив, который нужен, чтобы видеть старый остаток на балансе. За один клик до десяти групп, журнал кварталов сам не стартует. В Alfa ничего не создаёт и не удаляет. Группы 2019 года, которых нет в ссылках карточки ученика, эта кнопка не увидит. Если отчёт пишет «ещё осталось» — нажмите снова. Для полного архива филиала есть соседняя, более широкая кнопка.",
  ...
  loadOneMoney: "Красная кнопка идёт по ученикам слева по одному. Кассу этого человека дочитывает до конца: если Alfa отдала не все страницы или не ответила — пауза 5 с и снова тот же id, к следующему не переходит. Календарь подгружает, только если его ещё нет. Сумма списания по номеру занятия один раз. Окно лет режет занятия, не платежи. В Alfa оплаты не создаёт. Стоп прерывает очередь.",
  ...
  tabAudit: "Четвёртый шаг — сверка остатка. Берёт всех, кто сейчас учится, по одному, пауза 1 секунда. Для каждого читает из Alfa общий остаток с шапки карточки (customer.balance), считает число в «Клиентах» и кассу оплаты минус проведённые. Rest абонемента рядом как расшифровка, не эталон. Если цифры разошлись — добирает журнал или кассу только этого человека и считает снова. В Alfa ничего не пишет и не подставляет чужую цифру в файл. Совпало — карточка справа, даже если в календаре есть будущий урок с ценой. Не совпало — слева с причиной: мало явок, мало оплат, ошибка показа. «Сверить 10» — проба, не замена полного прогона.",
  auditAll: "Красная кнопка проходит всех текущих учеников с диска, статус «учится». Архивных и лидов нет. Между людьми пауза одна секунда, в Alfa один запрос в полёте. Сравнивает число на карточке «Клиенты» с общим остатком шапки Alfa, не с rest одного абонемента. Если не сошлось — догружает явки или оплаты только этого номера и считает ещё раз. Цифру из Alfa в кассу не записывает. Стоп прерывает после текущего. Прогресс в серой строке справа: имя, Клиенты, Alfa, касса, код. Запланированный урок с суммой сам по себе не держит карточку слева.",
  auditTen: "Проба: десять случайных текущих, не весь список. Тот же такт, что у «Сверить всех»: чтение шапки Alfa, сравнение, при расхождении добор только этого id. Нужна, чтобы быстро увидеть, сходится ли показ, не гоняя 345 человек. В Alfa не пишет. Если слева после пробы жёлтые с кодом formula — скорее показ в «Клиентах», а не дырявый журнал. Полный прогон — соседняя красная кнопка. Повтор можно жать, выборка каждый раз новая.",
...
export function AdminCrmSettings() {
  ...
  async function pullArchiveCatalog() {
    ...
    if (!more) {
      if (
        !window.confirm(
          "Архив клиентов: по одной карточке, пауза 1 с. Календарь и касса не качаются. Без живого ФИО не пишем. Стоп — после текущей. Продолжить?",
        )
      )
        return;
    }
