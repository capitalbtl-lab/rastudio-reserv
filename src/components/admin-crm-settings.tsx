...
const HINT = {
  ...
  loadOnePeople: "Красная очередь слева, по одному, пауза 5 с. Сверяет набор номеров уроков, не только 244/254. Нет чужих id — вправо. Есть id, которых нет на диске — дописывает. Окно лет — список в этой же красной рамке. Синяя рамка не читается. «Стоп» после текущего. В Alfa не пишет.",
  years: "Только красные кнопки шага 2: массовая «Загрузить по одному», на карточке «Загрузить календарь» и «Добрать». Стоит в красной рамке. Не синяя, не медленный автодобор, не «вся история» (там всегда 2015), не «сверить счёт». Пауза между детьми всегда 5 с — быстрее нельзя. По умолчанию «с начала · 2015». Короче — если человек ходит недавно. В Alfa ничего не отправляет.",
  ...
  recheckOnePeople: "Сначала правый столбец, окно ±. Если слева остались жёлтые — добирает их (не окно). Кто добрался — снова справа тем же окном. Пауза: 2 недели и меньше — 1 с, месяц — 2 с, 3 мес — 3 с, 6 мес — 4 с, 3 года / 7 лет / 2015 — 5 с. Стоп после текущего. В Alfa не пишет.",
  recheckWindow: "Только синие кнопки в этой рамке: массовая «Перепроверить по одному» и «Перепроверить» на карточке справа. ± неделя / ± 2 недели / ± месяц / ± три / ± шесть — обе стороны от сегодня. «За 3 года», «за 7 лет», «с начала · 2015» — назад от сегодня плюс месяц вперёд (запланированные). Пауза: ≤2 нед — 1 с, месяц — 2 с, 3 мес — 3 с, 6 мес — 4 с, длинные окна — 5 с. Старше выбранного окна не ищем и с диска не стираем. Красная рамка это окно не читает. В Alfa не пишет.",
  ...
  grain: "Только красная рамка шага 3 и кнопки порции на карточке группы. «Квартал» — самое безопасное: одно нажатие не закрывает всю историю. «Полугодие» больше. «Год» — у молодых групп. Срок группы режет лишнее. Это не «с 2015» и не окно синей рамки. Синяя рамка зерно не читает. В Alfa ничего не отправляет.",
  ...
  loadOneMoney: "Красная очередь слева на шаге 4: касса, по одному. Платежи по id, все виды. Журнал не качает. Годы в рамке режут Alfa: «2015» — с 2015-01-01, «1 год» — за год. Лиды из групп шага 1 тоже. Направо, когда страницы дочитаны. Сумма — шаг 5. В Alfa оплаты не создаёт. Стоп после текущего.",
  yearsMoney: "Только красные кнопки шага 4. «С начала · 2015» — платежи с 2015-01-01. «7 / 3 / 1» — с этой даты по сегодня. Синяя рамка этот список не читает: у неё своё окно ±. В Alfa ничего не отправляет, только фильтр чтения.",
  loadPay: "Красная касса этой карточки. Платежи и абонементы из Alfa на диск, по id. Журнал не качает. Годы рамки режут чтение. Дубли по номеру не плодит. После дочитанных страниц карточка вправо. В Alfa платёж не проводит.",
  ...
  recheckOneGroups: "Сначала правые группы, окно ±. Потом недогруженные слева (квартал, не окно). Кто добрался — снова справа тем же окном. Пауза как у синей: ≤2 нед — 1 с, месяц — 2 с, 3 мес — 3 с, 6 мес — 4 с, длинные окна — 5 с. Стоп после текущей. В Alfa не пишет.",
  recheckOneMoney: "Сначала справа касса по id. Потом слева недочитанная. Кто дочитался — снова справа. Сумма — шаг 5. Журнал не качает. Пауза как у синей: ≤2 нед — 1 с, месяц — 2 с, 3 мес — 3 с, 6 мес — 4 с, длинные окна — 5 с. Стоп после текущего. В Alfa не пишет.",
  ...
  tabMoney: "Четвёртый шаг — касса: платежи по id, не уроки, не остаток. Товар (тип 2 и 9) не грузим — ломает шапку уроков. Те же люди, что шаг 1–2. Сумма vs шапка — шаг 5. В Alfa оплаты не создаёт.",
