...
const HINT = {
  ...
  years: "По умолчанию «С начала · 2015» — вся история, как сверка счёта. «7 лет», «3 года» и «1 год» короче, только если человек ходит недавно. Действует на красную загрузку, синюю перепроверку и «Добрать». В Alfa ничего не отправляет.",
  ...
  recheckCal: "Ещё раз спрашивает Alfa по этому ученику: дырки дописывает, лишние строки с диска снимает. Тема и ДЗ при слиянии не затираются. Свои уроки в очереди на Alfa не трогает. В Alfa ничего не сохраняет и не удаляет. Если на диске занятий больше, чем в Alfa — жёлтая «есть дубли»; после снятия цифры должны совпасть. Можно жать точечно, не гоняя всю очередь слева.",
  ...
  recheckOnePeople: "Синяя «Перепроверить по одному» идёт только по тем, кто уже справа. По одному человеку сверяет журнал с Alfa: дырки добрать, дубли с диска снять. Между людьми пауза пять секунд. «Стоп» прерывает после текущего — сама снова не поедет. Если очередь оборвалась и Стоп не жали, через 30 секунд без движения продолжит с того же человека, не сначала. В Alfa ничего не пишет. Красная загрузка слева эту чистку не делает.",
