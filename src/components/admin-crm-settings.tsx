...
function BtnCluster({
  ...
  const lab = tone === "red" ? "text-red-800" : "text-sky-900";
  return (
    <span className={cn("inline-flex flex-wrap items-center gap-1 rounded-2xl p-1 ring-1", cls)}>
      <span className={cn("px-2 text-[0.62rem] font-bold uppercase tracking-wide", lab)}>{label}</span>
      {children}
    </span>
  );
}
...
const HINT = {
  ...
  loadOnePeople: "Красная очередь слева, по одному, пауза 5 с. Сверяет набор номеров уроков, не только 244/254. Нет чужих id — вправо. Есть id, которых нет на диске — дописывает. Окно лет — список «годы · красная» в этой же красной рамке. Список «месяцы · синяя» не читает. «Стоп» после текущего. В Alfa не пишет.",
  years: "Только красные кнопки шага 2: массовая «Загрузить по одному», на карточке «Загрузить календарь» и «Добрать». Не синяя, не медленный автодобор, не «вся история» (там всегда 2015), не «сверить счёт». По умолчанию «с начала · 2015». Короче — если человек ходит недавно. В Alfa ничего не отправляет.",
  probe: "Перепись уникальных номеров занятий по филиалам 1–4 с 2015. Не качает строки. Не смотрит ни «годы · красная», ни «месяцы · синяя». Пустой ответ Alfa не обнуляет счёт. Набор id разный — жёлтая. Сошлись — вправо. В Alfa не пишет.",
  recheckCal: "Синяя этой карточки. Окно — «месяцы · синяя» в синей рамке сверху (месяц / 3 / 6), не список годы. Сравнивает номера уроков в окне. Новые дописывает, ушедшие в окне снимает. Старше окна не трогает. Свои уроки и очередь не снимает. В Alfa не пишет.",
  recheckOnePeople: "Синяя очередь справа, по одному, пауза 5 с. Окно — список «месяцы · синяя» в этой же синей рамке. Сравнивает номера, не голую цифру. Новое в окне дописывает, удалённое в окне снимает. Старше окна не ищет. Красную слева не запускает. «Годы · красная» не читает. Стоп после текущего. В Alfa не пишет.",
  recheckWindow: "Только синие кнопки шага 2: массовая «Перепроверить по одному» и «Перепроверить» на зелёной карточке. Месяц ≈ 32 дня, три ≈ 92, шесть ≈ 182 — по дате урока. Красная, медленный автодобор, «Добрать» и «вся история» это окно не берут. Дальше окна — красная или «С нуля». В Alfa не пишет.",
  ...
  fullHist: "Только жёлтая. Та же красная качка, что «Добрать», но всегда с 1 января 2015 — список «годы · красная» не читает. После шага колонка смотрит набор id. Не синяя и не месяцы. В Alfa не пишет.",
  ...
  loadCal: "Красная качка этой карточки. Окно лет — «годы · красная» рядом, не месяцы. Сверяет набор номеров, дописывает дырки. Старая жёлтая — лучше «Загрузить всю историю» (всегда 2015). После шага колонка по id. В Alfa не пишет.",
...
function PeopleFillList({
  ...
  function renderPerson(row: PeopleRow) {
    ...
    if (row.alfa != null && Number.isFinite(Number(row.alfa))) {
      if (Boolean(row.short) && Number(row.lessons) === 0) alfaKeep.current[row.cid] = Number(row.alfa);
          ...
          {short && kind === "students" && onResetHistory
            ? withHint(
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    if (!confirm(`№${row.cid}: стереть занятия с диска, заново спросить счёт Alfa и качать с 2015?`)) return;
                    onResetHistory(row);
                  }}
                >
                  С нуля
            ...
            <div className="space-y-3">
              ...
              <section className="rounded-2xl bg-surface-2 p-4 ring-1 ring-black/8">
                ...
                <p className="mt-1 text-sm text-muted">
                  Красная рамка — загрузка и годы. Синяя рамка — перепроверка и месяцы. Они не общие.
                </p>
                <div className="mt-3">
                  <ScopePills
                ...
                {(() => {
                  ...
                  return (
                    <>
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-2">
                        <BtnCluster tone="red" label="красная · годы">
                        {withHint(
                        <button
                          type="button"
                        ...
                        <YearsSelect value={peopleFromId} disabled={busy} onChange={setPeopleFromId} tag="годы · красная" />
                        </BtnCluster>
                        <BtnCluster tone="sky" label="синяя · месяцы">
                        {withHint(
                        <button
                          type="button"
                          className={BTN_LOAD}
                      ...
                      <PeopleFillList
                        ...
                        years={<YearsSelect value={peopleFromId} disabled={busy} onChange={setPeopleFromId} small tag="годы · красная" />}
