...
const HINT = {
  ...
  archCatalog: "На виду три фильтра: возраст от–до, только с ФИО, были группы. Пустые — пишем всех архивных с живым именем. Телефон, число и «тест» не пишем никогда. Одна карточка, пауза 5 с, Стоп после текущей. Записанный сразу появляется слева, в «требуют загрузки». «Группы» — поле на карточке Alfa, не журнал уроков. В Alfa не пишет.",
...
export function AdminCrmSettings() {
  ...
  async function pullArchiveCatalog() {
    ...
    if (!more) {
      if (
        !window.confirm(
          "По одной карточке, пауза 5 с. Без живого ФИО не пишем. Старые не удаляем. Рабочий набор и журнал не трогаем. Стоп — после текущей. Продолжить?",
        )
      )
        return;
    }
    ...
    try {
      ...
      for (;;) {
        ...
        if (!res.more) break;
        if (stopSchool.current) break;
        setFillLoading({ kind: "archiveCatalog", label: `пауза 5 с · ${cat?.name || cat?.step || "дальше"}` });
        await pauseFive();
      }
    } finally {
      holdFill.current = false;
