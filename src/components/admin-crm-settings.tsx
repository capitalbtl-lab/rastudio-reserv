...
const HINT = {
  ...
  plan: "Пульт — это не вторая очередь. Это разрешение само запускать те же шаги 1–5 по расписанию и кнопка «запустить сейчас». Грузит тот же процесс «истории». Тумблер «Синхронизация расписания» включает слоты (например в 4:00). Выключили — слоты молчат, ручная кнопка «1–5 сейчас» всё равно работает. «Лог» рядом со сбросом: кто грузился, на ком оборвалось, почему слот пропустили. «Стоп» — на ленте шага, не в пульте. Пока кто-то уже грузится, слот ночью напишет «руки заняли» и не начнёт вторую очередь. В Alfa пульт ничего не пишет.",
  ...
  recheckOnePeople: "Синяя очередь. Сначала правый столбец (уже зелёные) в выбранном окне дат. Если слева остались жёлтые — добирает их целиком, не окном. Кто добрался — снова проверяет справа тем же окном. Пауза между людьми: 2 недели и меньше — 1 секунда, месяц — 2, три месяца — 3, полгода — 4, три года / семь лет / с 2015 — 5 секунд. Быстрее нельзя. «Стоп» после текущего. В Alfa не пишет.",
  recheckWindow: "Только синие кнопки в этой рамке. «± неделя / две / месяц / три / шесть» — смотрим в обе стороны от сегодня. «За 3 года», «за 7 лет», «с начала · 2015» — назад от сегодня и ещё месяц вперёд (запланированные). Пауза: чем короче окно, тем короче пауза (от 1 до 5 секунд). Старше окна не ищем и с диска не стираем. Красная рамка это окно не читает. В Alfa не пишет.",
  ...
  recheckOneGroups: "Синяя очередь групп. Сначала правые (уже зелёные) в окне дат. Потом недогруженные слева — их кварталом, не окном. Кто добрался — снова справа тем же окном. Пауза как у синей: от 1 до 5 секунд в зависимости от окна. «Стоп» после текущей. В Alfa не пишет.",
  ...
  tabGroups: "Шаг 3. Явки по группе: кто был на уроке в этом квартале. Это не личный календарь человека (он на шаге 2). Сначала красная «по одному» и размер порции: квартал / полугодие / год. Синяя — перепроверить окно дат. Архив групп и сроки курса — отдельные кнопки, их лучше нажать до массовой качки. Тема и домашка — в карточке группы, только после зелёных явок. Фильтр школы сужает очередь. В Alfa журнал не проводим.",
