...
const HINT = {
  ...
  recheckCal: "Синяя кнопка этой карточки. Окно дат — список в синей рамке сверху, не красная. Сверяет номера уроков только в этом окне один к одному: новые дописывает, пропавшие в окне снимает, проведённые перечитывает (явка, сумма, ноль из Alfa остаётся нулём). Что старше окна — не трогает. Свои ещё не отправленные уроки не снимает. В Alfa не пишет.",
    ...
    return {
      ...
      rec: /нет А/.test(extra)
        ...
        : /кассы нет/.test(extra)
          ? "Касса не закрыта шагом 4. Пустая лента при А — нули, шапку зовём."
