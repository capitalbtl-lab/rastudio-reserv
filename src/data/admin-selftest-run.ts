...
import { GROUPS_SECTION, GROUPS_PROBES } from "./admin-selftest-groups";
...
export const ADMIN_SECTIONS: SectionDef[] = [
  ...
  { id: "agent-guides", title: "Разделы сайта", hint: "Инструкции ИИ по управлению разделами кабинета. Карта ID расписания." },
...
const LINKS: Record<string, string[]> = {
  ...
  agent: ["agent", "agent-window", "agent-dialog", "agent-voices", "agent-edits", "agent-chats", "agent-train", "agent-guides", "agent-access", "agent-debug", "apis", "prices", "schedule"],
  ...
  "agent-guides": ["agent-guides", "agent", "schedule"],
...
const PROBES: Probe[] = [
  ...
  },
  {
    ...
    run: async () => {
      ...
      return ok(`КБМ ${f.kbm.mode} ${f.kbm.value}, ТМХ ${f.tmx.mode} ${f.tmx.value}, клиентов ${f.extra?.length || 0}`, ["prices"]);
    },
  },
  {
    id: "schedule-file",
  ...
  },
  {
    ...
    run: async () => {
      ...
      if (!store.items.length) {
        return fail(
          "нет абонементов",
          "Вкладка «Абонементы» пустая. Группам не к чему привязать тариф.",
          "Расписание → Абонементы → Загрузить из AlfaCRM.",
          ["schedule"],
        );
      }
      const active = store.items.filter((t) => !t.archive);
      const withSub = active.filter((t) => t.subjectIds.length).length;
      const noCard = store.items.filter((t) => !t.cardOk).length;
      const byBr = [2, 1, 3, 4]
        .map((id) => `${id}:${active.filter((t) => t.branchIds.includes(id)).length}`)
        .join(" ");
      return ok(
        `всего ${store.items.length}, активных ${active.length}, с предметами ${withSub}, филиалы ${byBr}, карточка CRM не прочиталась ${noCard}`,
        ["schedule"],
      );
    },
  },
  ...
  {
    id: "schedule-crm-push",
    title: "Выгрузка в AlfaCRM (запись)",
    sections: ["schedule"],
    run: async () => {
      const { listAdminSlots } = await import("./alfacrm-schedule");
      const s = listAdminSlots().find((x) => x.groupId && x.branchId && x.lessonId && x.subjectId);
      if (!s) return fail("нет группы для записи", "Нечего выгрузить: нет группы с gid, предметом и уроком.", "Загрузить из AlfaCRM.", ["schedule", "apis"]);
      try {
        const { token, request } = await import("./alfacrm");
        const t = await token();
        const g = await request<{ success?: boolean; model?: { id?: number; name?: string } }>(
          `/v2api/${s.branchId}/group/update`,
          { id: s.groupId, name: s.groupName },
          t,
        );
        if (!g?.model?.id && g?.success === false) {
          return fail("group/update не принял id", "Выгрузка существующих групп падает: AlfaCRM ждёт id в адресе, не в теле.", "Проверка кабинета. Если снова сбой — вставьте отчёт в Grok.", ["schedule", "apis"], JSON.stringify(g).slice(0, 400));
        }
        const les = await request<{ success?: boolean; model?: { id?: number } }>(
        ...
        return ok(
          `запись ок. group/update gid ${g.model?.id || s.groupId}, regular-lesson ${les.model?.id || s.lessonId}. Имя и время те же, новые группы не создавали.`,
          ["schedule", "apis"],
        );
      } catch (e) {
        const raw = e instanceof Error ? e.message : String(e);
        return fail(
          raw.slice(0, 200),
          "Выгрузка в CRM не проходит. «Выгрузить в AlfaCRM» и сохранение карточки группы не запишут изменения.",
          "Роль API: group/update и regular-lesson/update. id должен быть в адресе ?id=.",
          ["schedule", "apis"],
          raw,
        );
      }
  ...
  },
  {
    ...
    run: async () => {
      ...
      try {
        ...
        await ping("абонементы", "/v2api/2/tariff/index");
  ...
  {
    id: "section-guides",
    title: "Инструкции по разделам сайта",
    sections: ["agent", "agent-guides", "schedule"],
    run: async () => {
      const { loadGuides } = await import("./agent-section-guides-run");
      const list = loadGuides();
      const sch = list.find((g) => g.id === "schedule");
      if (!sch) {
        return fail(
          "нет инструкции расписания",
          "В Ассистент ИИ → Разделы сайта нет карточки «Расписание занятий». Агент расписания не знает Карту ID.",
          "Ассистент ИИ → Разделы сайта. Должна быть заводская инструкция schedule.",
      ...
      if (!sch.body.includes("groupId") || !sch.body.includes("courseId")) {
        return fail(
          "инструкция без ID",
          "Текст инструкции расписания не содержит groupId/courseId. Агент снова начнёт угадывать по названию.",
          "Разделы сайта → Расписание занятий → Вернуть заводской.",
          ["agent-guides", "schedule"],
...
export function formatGrokReport(opts: { title: string; section: string; pass: number; fail: number; checks: CheckResult[]; dry?: boolean }) {
  const leftovers = [...new Set(opts.checks.map((c) => c.leftover).filter(Boolean))];
  const fails = opts.checks.filter((c) => !c.ok && !c.skip);
  const lines = [
    ...
    opts.dry === false
      ? "Режим: запись в AlfaCRM. Удаление в CRM не выполнялось — оператор удаляет тестовые записи сам."
      : "Ничего не записывали в CRM и на сайт.",
    ``,
  ];
  if (fails.length) {
    lines.push("ОШИБКИ ДЛЯ ОПЕРАТОРА:");
    for (const c of fails) {
      lines.push(`• ${c.title}: ${c.plain || c.detail}`);
      if (c.fix) lines.push(`  как чинить: ${c.fix}`);
    }
    lines.push("");
  }
  if (leftovers.length) {
    lines.push("ОПЕРАТОРУ: удалите в AlfaCRM сами (проверка ничего не стирает):");
    for (const row of leftovers) lines.push(`• ${row}`);
    lines.push("");
  }
...
export async function runAdminSelfTest(data: { token?: string; section?: string }) {
    ...
    const dry = section !== "schedule-groups";
    const leftovers = [...new Set(checks.map((c) => c.leftover).filter((x): x is string => Boolean(x)))];
    const errors = checks.filter((c) => !c.ok && !c.skip).map((c) => `${c.title}: ${c.plain || c.detail}`);
    const grok = formatGrokReport({ title, section, pass, fail: failN, checks, dry });
    return {
      ok: true as const,
      section,
      title,
