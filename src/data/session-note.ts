...
const ASK: Record<NoteField, RegExp> = {
  age: /сколько.{0,24}лет|какой возраст|возраст реб|нажмите[^.]*лет|скажите[^.]*лет|сначала возраст|сколько ребёнк/i,
...
function nextFromNote(n: {
  ...
  if (!n.age) {
    ...
    return "Спроси ТОЛЬКО возраст ребёнка одной короткой фразой и жди. Город не упоминай.";
  }
  if (!n.city) {
    if (n.asked.city) {
    ...
    return "Подтверди возраст коротко и спроси ТОЛЬКО город: Коломна или Луховицы. Возраст не спрашивай.";
  }
  ...
  if (!n.school) {
    ...
    return "Спроси направление узко: 2–3 варианта по возрасту, не список всех школ. Город, филиал и возраст не спрашивай.";
  }
  if (!n.course && !n.briefed) {
    return "Два предложения про направление. Спроси одно: подробнее или сразу пробное. Город не спрашивай.";
  }
  if (!n.briefed) {
