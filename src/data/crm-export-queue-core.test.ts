...
describe("очередь выгрузки в Alfa", () => {
  ...
  it("снятие абонемента схлопывается, занятие уходит своим путём", () => {
    ...
    const sameLocal = mergeExportJob(
      mergeExportJob([], { op: "pay.create", branchId: 1, entityId: 670, body: { customer_id: 670, income: 1, localId: -9 } }),
      { op: "pay.create", branchId: 1, entityId: 670, body: { customer_id: 670, income: 1, localId: -9, pay_item_id: 2 } },
    );
    assert.equal(sameLocal.length, 1);
  ...
  it("смена этапа локального лида складывается в customer.create, не уходит с id < 0", () => {
    ...
    assert.equal(canRunExportJob({ op: "lesson.create", entityId: -12 }), false);
    assert.equal(canRunExportJob({ op: "lesson.create", entityId: -12, body: { customer_ids: [-12] } }), false);
    assert.equal(canRunExportJob({ op: "lesson.create", entityId: 670, body: { customer_ids: [670], localId: -5 } }), true);
