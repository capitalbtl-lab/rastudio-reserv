...
describe("форма занятия карточки", () => {
  it("филиал и аудитория обязательны, группы — после филиала", () => {
    ...
    assert.match(strip, /Забыли провести\?/);
    assert.match(strip, /Проведен без списания/);
    assert.match(strip, /Запланирован бесплатный/);
    assert.match(strip, /Пропуск оплач/);
    assert.match(strip, /TILE_LEGEND/);
    ...
    assert.match(strip, /statusId: asConduct/);
    ...
    assert.match(strip, /writeAttendCols\(colWRef\.current\)/);
    assert.match(strip, /data-op="lesson-conduct-btn"/);
    ...
    assert.match(inbound, /customer_id: id, date_from: dateFrom, date_to: dateTo/);
