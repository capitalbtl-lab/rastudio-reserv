...
Кто ведёт разговор — зависит от темы. Второй коротко поддакивает. Олег — мужской род: согласен, готов, понял. Ольга — женский: согласна, готова, поняла. Никогда не пишите «Ольга: … согласен».
...
export const chatAgent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const adminHint = admin
      ? `\n\nСобеседник УЖЕ администратор, доступ открыт. Сейчас страница: ${data.path || "/"}.
Режим управления сайтом: отвечает ТОЛЬКО Ольга. Формат ответа: одна или несколько строк «Ольга:». Строк «Олег:» быть не должно. Олег молчит.
НИКОГДА не проси кодовое слово снова. Сразу работай с правками: цены, тексты, голоса. Если спрашивают войти — «режим управления уже открыт, что меняем?»`
      : "";
