...
describe("ручной журнал с Alfa", () => {
  ...
  it("только кнопка группы и порция, фон сам не качает", () => {
    ...
    assert.match(ui, /role === "all" && who === "архив" && hasClients/);
    assert.match(pull, /rankedStudentIds\("1"\)\.map\(\(x\) => x\.cid\)/);
    ...
    assert.doesNotMatch(cashFn, /censusCustomerLessonIds/);
    assert.doesNotMatch(cashFn, /inboundMissingCustomerLessons/);
    assert.doesNotMatch(cashFn, /applyCustomerLessonCensus/);
    assert.doesNotMatch(cashFn, /inboundCustomerLessons/);
  ...
  it("проба не стирает известный счёт Alfa и не закрывает дырку", () => {
    ...
    assert.doesNotMatch(inbound, /if \(!fillDone && walked && keep > diskNow/);
    assert.doesNotMatch(inbound, /cur.done && !monthly\) \{\s*cur = lessonFillStart/);
  ...
  it("3g: окно не 2015 если счёт есть; missing сажает или снимает seen", () => {
    ...
    assert.match(one, /refreshStaleOf/);
    assert.match(one, /if \(await refreshStaleOf\(first.ok/);
    assert.match(one, /studentIndexBranches/);
    assert.match(one, /if \(cardOnly\) \{/);
    assert.match(one, /studentIndexBranches\(branchId, cid/);
    assert.doesNotMatch(one, /studentCardBranches\(cid, branchId\)/);
    assert.match(inbound, /export async function studentIndexBranches/);
    assert.match(inbound, /\/v2api\/\$\{homeN\}\/customer\/index/);
    assert.match(inbound, /\{ id, page: 0 \}/);
    assert.match(inbound, /studentIndexBranchIds\(homeN, id/);
