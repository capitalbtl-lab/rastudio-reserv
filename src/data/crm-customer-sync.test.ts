...
describe("карточка не ждёт Alfa", () => {
  ...
  it("журнал ученика: полная история один раз, дальше окно и union", () => {
    ...
    assert.match(inbound, /pull\(1, dateFrom, dateTo\)/);
    ...
    const chunk = pay.slice(payAt, payAt + 14000);
    ...
    assert.match(chunk, /payFillRange\(opts\?\.dateFrom, opts\?\.dateTo\)/);
    assert.match(chunk, /\.\.\.fillDates/);
    assert.match(chunk, /customer_id: customerId,\s*\.\.\.fillDates/);
