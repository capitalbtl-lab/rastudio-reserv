...
const SCHEDULE_OPS: GuideOp[] = [
  ...
  { id: "assign-lesson", title: "Назначить занятие", body: "lesson-dialog / book_lesson. subjectId* со слота. Диск календаря сразу (lessonId < 0), очередь lesson.create. Правила Alfa: group/makeup/extra/overtime — group_ids можно. trial/intro/individual/разовые — БЕЗ group_ids, зал не слать если занят. cgi.apply только для постоянной группы. Нет subjectId — ошибка." },
  { id: "book-trial", title: "Записать на пробное", body: "consultantCanBook. submit_trial или book_lesson lesson_type=trial. parent, child, phone, branch_id. Слот: date, time, subject_id, teacher_id. gid только чтобы взять время слота — в Alfa group_ids НЕ слать (тип 3). Зал если занят — без room_id. Диск календаря сразу." },
  { id: "book-group", title: "Записать в группу", body: "book_lesson lesson_type=group. gid обязателен. group_ids + cgi.apply. Приоритет 0 с сайта не записывать. Не пробное, если просят абонемент." },
  { id: "book-makeup", title: "Отработка", body: "Узнанный клиент. Неделя → три слота того же курса у того же педагога. book_lesson makeup, тип 4, gid слота, group_ids можно, cgi не писать. Не пробное." },
  { id: "book-teacher-slot", title: "Индивидуальное и сверхурочное", body: "book_lesson individual|overtime. teacher_id обязателен, дата, время. Без группы. extra — дата/время/subject_id." },
  { id: "book-oneoff", title: "Разовые события", body: "book_lesson master|open|excursion|camp|event|interview|aftercare|summer. Без group_ids. Точный lesson_type, не выдумывать." },
  ...
  { id: "ai-roles", title: "Роли ИИ", body: "Ассистент ИИ → Окно: ROLE_FLAGS и BOOK_TYPE_FLAGS, группы LESSON_POLICY_GROUPS. Консультант ≠ голос админки. Типы: trial 3 без group_ids, group 2 + cgi, makeup 4 gid без cgi, individual/overtime teacher_id, разовые без группы. Не смешивать миры без галочки." },
...
function scheduleBody() {
  ...
  return `ИНСТРУКЦИЯ РАЗДЕЛА «Расписание занятий» для ИИ. Метод: Карта ID. REV ${GUIDE_REV}. Точка восстановления: ромашка 9.
...
Точка восстановления: ромашка 9.
