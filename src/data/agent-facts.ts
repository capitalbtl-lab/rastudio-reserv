...
export function nextStepOf(facts: SessionFacts) {
  ...
  if (facts.mode === "client") {
    ...
    if (facts.intent === "второй") {
      return "это второй ребёнок той же семьи. Не открывать карточку первого. Спросить имя и возраст второго, затем пробное на телефон родителя. Не воронка с нуля без телефона.";
    }
