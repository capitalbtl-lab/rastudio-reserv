...
export function nextStepOf(facts: SessionFacts) {
  ...
  if (facts.mode === "client") {
    ...
    if (facts.intent === "абонемент") {
      return "ребёнок уже подтверждён. Сказать абонемент и остаток с диска. Если можно вешать — кнопки tariffId, не имя шаблона. Не переспрашивать имя. Не воронка нового.";
    }
  ...
  if (!facts.age) return "спросить ТОЛЬКО возраст. Можно сказать, написать или нажать кнопку. Формула переспроса: «Чтобы не предложить слишком сложное, скажите или напишите возраст или нажмите кнопку.» Город не упоминать.";
...
export function talkFallback(who: "oleg" | "olga", facts: SessionFacts) {
  ...
  if (!facts.mode || facts.mode === "fork") {
    ...
    return `${n}: Нажмите кнопку или скажите: уже ходим, подбираем впервые — или вас интересуют правила и цены.`;
  }
  if (facts.school) {
  ...
  if (!facts.age) return `${n}: Чтобы не предложить слишком сложное, скажите или напишите возраст или нажмите кнопку.`;
