...
export const ROLE_FLAGS: { id: keyof AgentSettings; title: string; hint: string; tip: string }[] = [
  {
    ...
    hint: "Олег/Ольга вызывают book_lesson (пробное, группа, отработка и остальные типы ниже). submit_trial = пробное.",
  ...
  },
  {
    ...
    hint: "Может сказать «войти в административный режим», если человек назвался сотрудником. Сама ссылка внизу чата — галочка «Административный режим из чата» выше.",
  ...
  {
    id: "consultantCanSkip",
    title: "Консультант отмечает пропуск",
    hint: "«Не придём» — note_skip в карточку и ленту. Явку группы не переписывает.",
    tip: "Выключено — пропуск принимает администратор по телефону 8 (800) 511-34-01.",
  },
  {
    id: "consultantCanPause",
    title: "Консультант ставит паузу",
    hint: "Пауза занятий до даты — pause_classes. Не путать с пропуском одного урока.",
    tip: "Выключено — паузу ставит администратор по телефону. Пропуск при этом может быть включён отдельно.",
  },
  {
...
export function loadBrain(): Brain {
  try {
    ...
    const merged = mergeAgentSettings(raw.settings || {});
...
export function rolesPrompt(kind: "site" | "admin") {
  ...
  if (kind === "site") {
    return [
      ...
      `Запись: ${s.consultantCanBook !== false ? "МОЖНО book_lesson (пробное = lesson_type=trial или submit_trial)." : "НЕЛЬЗЯ записывать. Назови слоты и телефон 8 (800) 511-34-01"}.`,
      bookTypesPrompt(s),
      ...
      `Пропуск: ${s.consultantCanSkip !== false ? "МОЖНО note_skip (одно занятие). Не путать с паузой." : "НЕЛЬЗЯ. Только телефон 8 (800) 511-34-01."}`,
      `Пауза: ${s.consultantCanPause !== false ? "МОЖНО pause_classes до даты. Не путать с пропуском." : "НЕЛЬЗЯ. Только телефон 8 (800) 511-34-01."}`,
      `Абонемент: ${s.consultantCanTariff ? "МОЖНО assign_tariff по tariffId." : "НЕЛЬЗЯ вешать абонемент. Это кабинет сотрудника."}`,
...
export const adminAgentBrain = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (data.action === "saveSettings") {
      ...
      brain.settings = mergeAgentSettings({
        ...brain.settings,
        ...next,
        defaultPartner: next.defaultPartner === "oleg" ? "oleg" : "olga",
        style: next.style === "short" || next.style === "detailed" || next.style === "warm" ? next.style : brain.settings.style,
        extra: String(next.extra ?? brain.settings.extra).slice(0, 4000),
        askOnce: next.askOnce !== false,
        injectTraining: next.injectTraining !== false,
        maxExamples: Math.max(4, Math.min(80, Number(next.maxExamples || brain.settings.maxExamples || 40))),
        showChat: flag(next.showChat, brain.settings.showChat),
        allowVoice: flag(next.allowVoice, brain.settings.allowVoice),
        allowAdminMode: flag(next.allowAdminMode, brain.settings.allowAdminMode),
        showChips: flag(next.showChips, brain.settings.showChips),
        allowOlga: flag(next.allowOlga, brain.settings.allowOlga),
        allowOleg: flag(next.allowOleg, brain.settings.allowOleg),
        allowReset: flag(next.allowReset, brain.settings.allowReset),
        allowBarge: flag(next.allowBarge, brain.settings.allowBarge),
        matchChipsToMessage: flag(next.matchChipsToMessage, brain.settings.matchChipsToMessage),
        keepAssistantReplies: flag(next.keepAssistantReplies, brain.settings.keepAssistantReplies),
        speakEveryReply: flag(next.speakEveryReply, brain.settings.speakEveryReply),
        consultantCanBook: flag(next.consultantCanBook, brain.settings.consultantCanBook),
        consultantCanSeeAllGroups: flag(next.consultantCanSeeAllGroups, brain.settings.consultantCanSeeAllGroups),
        consultantCanManage: flag(next.consultantCanManage, brain.settings.consultantCanManage),
        adminVoiceCanWrite: flag(next.adminVoiceCanWrite, brain.settings.adminVoiceCanWrite),
        adminVoiceCanConsult: flag(next.adminVoiceCanConsult, brain.settings.adminVoiceCanConsult),
