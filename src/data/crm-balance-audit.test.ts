...
describe("шаг 5 сверка остатка", () => {
  ...
  it("модуль не пишет в Alfa, не качает кассу и журнал", () => {
    ...
    assert.match(src, /step5UnitScale/);
    assert.match(src, /unitScale/);
  ...
  it("карточка шага 5: товар не в формуле, живые списания, чипы пропуска", () => {
    ...
    assert.match(ui, /step5FitRemainder/);
    assert.match(ui, /Товар в ленте есть/);
    assert.match(ui, /дока ТМЦ/);
    ...
    assert.match(ui, /id === "goods"/);
    assert.match(ui, /unitScale/);
    assert.match(ui, /toFixed\(2\)\.replace/);
  ...
  it("товар в кассе, шапка без него — goods; корректировка с текстом не режет ₽", () => {
    ...
    assert.match(src, /step5FitRemainder/);
    assert.match(src, /chargeFromPupils/);
    assert.match(src, /lessonDebtLike/);
    assert.match(src, /step5DebtPrice/);
