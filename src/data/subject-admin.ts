...
export function packSubjectRows(list?: CrmSubject[]) {
  ...
  return {
    ...
    subjects: subjects.map((s) => {
      ...
      const courseId = canonCourseId(tree, link?.courseId || "");
...
export function bindSubjectCourse(subjectId: number, courseId: string) {
  const tree = loadSiteTree();
  const cid = canonCourseId(tree, courseId);
  const course = cid ? tree.courses.find((c) => c.id === cid) : undefined;
