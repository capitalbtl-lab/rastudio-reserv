...
describe("макет главной", () => {
  ...
  it("главная оборачивает каждый блок и правит текст", () => {
    ...
    const publicCss = readFileSync(new URL("../styles.css", import.meta.url), "utf8");
    assert.doesNotMatch(publicCss, /html\.home-editing \.mobile-dock/);
    assert.doesNotMatch(publicCss, /\.ve-media-ghost/);
